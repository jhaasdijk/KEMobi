...
# This file was *autogenerated* from the file roots_cyclic.sage
from sage.all_cmdline import *   # import sage library

_sage_const_17 = Integer(17); _sage_const_8 = Integer(8); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2)
...
# Global parameters
VAR_Q: int = _sage_const_17 
VAR_P: int = _sage_const_8 
...
def bitreverse(numbers: Vector, width: int) -> Vector:
    ...
    return [int(fmt.format(_)[::-_sage_const_1 ], _sage_const_2 ) for _ in numbers]
...
def root_primitive(n: int) -> int:
    ...
    for i in range(_sage_const_1 , VAR_Q):
...
def roots_cyclic() -> (Vector, Vector):
    ...
    brv = bitreverse(list(range(int(VAR_P / _sage_const_2 ))), log(VAR_P, _sage_const_2 ) - _sage_const_1 )

    # Calculate the roots
    roots = []
    values = [psi ** brv[_] for _ in range(int(VAR_P / _sage_const_2 ))]
    for idx in range(log(VAR_P, _sage_const_2 )):
        layer = _sage_const_2  ** idx
        roots += values[:layer]

    # Calculate the inverse roots
    roots_inv = [pow(_, -_sage_const_1 , VAR_Q) for _ in roots]
...
def roots_negacyclic() -> (Vector, Vector):
    ...
    psi = root_primitive(_sage_const_2  * VAR_P)
    # Calculate the bitreversed indices
    brv = bitreverse(list(range(VAR_P)), log(VAR_P, _sage_const_2 ))

    # Calculate the roots, use range(1, VAR_P) to skip the first element
    roots = [psi ** brv[_] for _ in range(_sage_const_1 , VAR_P)]
    # Calculate the inverse roots
    roots_inv = [pow(_, -_sage_const_1 , VAR_Q) for _ in roots]
